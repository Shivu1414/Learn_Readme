...
use Webkul\Modules\Wix\WixmpBundle\Form\Export\SellerExportType;
use Sensio\Bundle\FrameworkExtraBundle\Configuration\ParamConverter;
use Symfony\Component\Translation\TranslatorInterface;
use Symfony\Component\Cache\Adapter\AdapterInterface;
use Webkul\Modules\Wix\WixmpBundle\Utils\SellerHelper;
/**
 * @Route("/export", name="wixmp_export_")
...
class ExportController extends BaseController
{
    ...
    public function __construct(TranslatorInterface $translator, AdapterInterface $cache, SellerHelper $sellerHelper)
    {
     ...
     */
    public function get_admin_export_product(Request $request, CompanyApplication $companyApplication, $formId = 'ExportForm', CatalogHelper $catalogHelper)
    {
        ...
        $export_product_url = $this->generateUrl('marketplace_export_product', ['storeHash' => $companyApplication->getCompany()->getStoreHash()]);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $return['redirect_url'] = $this->generateUrl('marketplace_export_get_file', $output_params);
    ...
    public function sample_product_csv(Request $request, CompanyApplication $companyApplication, CatalogHelper $catalogHelper)
    {
        ...
        $return['redirect_url'] = $this->generateUrl('wixmp_export_get_file', $output_params);
     ...
     * @ParamConverter("companyApplication", class="App\Entity\CompanyApplication", converter="company_application_converter")
     */
    public function sellerExport(Request $request, CompanyApplication $companyApplication,$formId = 'wixMpSellerExportForm', ImportExportHelper $importExportHelper)
    {
        $sellerHelper = $importExportHelper->getAppHelper("seller");
        $include_ids = [];
        $data = [
            'file_name' => 'sellers_'.rand().'.csv',
            'output_type' => 'D',
            'delimiter' => 'C',
        ];

        $seller_fields = $sellerHelper->getSellerFields();
        ...
        $export_seller_url = $this->generateUrl('wixmp_export_seller', ['storeHash' => $companyApplication->getCompany()->getStoreHash()]);
        $form = $this->createForm(SellerExportType::class, $data, ['action' => $export_seller_url, 'export_fields' => ['primary_fields' => $primary_fields, 'other_fields' => $other_fields]]);
        $form->handleRequest($request);
        //dd($form);
        if ($form->isSubmitted() && $form->isValid()) {
            $form_data = $form->getData();
            ...
            $return = $this->sellerHelper->handle_export_req($request, $params, $companyApplication);
            $output_params = [
                'storeHash' => $companyApplication->getCompany()->getStoreHash(),
                'filename' => $form_data['file_name'],
            ];
            ...
            return new JsonResponse($return);
        } elseif ($form->isSubmitted()) { //dd($form);
            return new JsonResponse(
                array(
                    'status' => '400',
                    'notifications' => [
        ...
        return $this->render('@wixmp_twig/view_templates/ImportExport/export_csv.html.twig', [
            'form' => $form->createView(),
            'formId' => $formId,
        ]);
